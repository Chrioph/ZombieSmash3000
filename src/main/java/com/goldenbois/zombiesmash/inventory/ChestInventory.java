...
public class ChestInventory implements Serializable{
    ...
    private Rectangle openingHitbox=new Rectangle(0,0,0,0);
    ...
    public void render(Graphics g){
        ...
        if ( activeList.equals(displayInventoryItems)){
            g.drawString("INVENTORY ITEMS", invListCenterX, invImageY-30);
        }
        if ( activeList.equals(chestInventoryItems)){
            g.drawString("CHEST ITEMS", invListCenterX, invImageY-30);
        }
        if(len == 0)
            return;
        ...
        for(int i = -5;i < 6;i++){
            if(selectedItem + i < 0 || selectedItem + i >= len)
                continue;
            if(i == 0){
                g.drawString("> " + activeList.get(selectedItem + i).getName() + " <", invListCenterX,
    ...
    public Rectangle getCollisionBounds(float xOffset, float yOffset) {
        return new Rectangle ((int) (openingHitbox.getX() + xOffset - handler.getGameCamera().getxOffset()),(int) (openingHitbox.getY() + yOffset - handler.getGameCamera().getyOffset()), openingHitbox.getWidth(), openingHitbox.getHeight() );
    }
}
