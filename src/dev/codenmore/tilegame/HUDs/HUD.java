...
public class HUD {
	...
	public void render(Graphics g) {
		if (!handler.getWorld().getEntityManager().getPlayer().getInventory().isActive()) {
			g.drawImage(Assets.HUDWindow, 5, 5, 600, 120, null);
			g.drawImage(Assets.heart, 30, 50,64,64, null);
			g.drawImage(Assets.sword, 144, 50,48,48, null);
			g.drawImage(Assets.arrow, 242, 50,48,48, null);
			...
			Text.drawString(g, ""+ handler.getWorld().getEntityManager().getPlayer().getAmmunition(), 267, 40, true, Color.WHITE, Assets.font40);
			Text.drawString(g, ""+ handler.getWorld().getEntityManager().getPlayer().getArmor(),      370, 40, true, Color.WHITE, Assets.font40);
			Text.drawString(g, "Mode",     															  480, 40, true, Color.WHITE, Assets.font40);
			if(handler.getWorld().getEntityManager().getPlayer().isRangedToggled())
				g.drawImage(Assets.bow, 454, 50,48,48, null);
			if(!handler.getWorld().getEntityManager().getPlayer().isRangedToggled())
				g.drawImage(Assets.sword, 454, 50,48,48, null);
		}
	}
}
